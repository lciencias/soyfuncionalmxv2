...
$array = array();
if(isset($_POST)  && (int) $_POST['idT'] > 0 && (int) $_POST['idT'] < 8 && ((int) $_SESSION['userId'] > 0) ){
	...
	$opcion = Comunes::SAVE;
	if((int) $_POST['id'] > 0){
		$opcion = Comunes::UPDATE;
	}
	switch((int)$_POST['idT']){
        case 1:
			$usuario = new Usuario($db,$_SESSION,$_POST,$_POST['idT'], $opcion);
		...
		case 2:
			...
			$slide = new Slider ($db,$_SESSION,$_POST,$idImagen,$opcion,$idImagenM);
			$array = array('exito' => $slide->obtenExito(),'msg' => $slide->obtenMensaje(), 'url' => $url);
			break;
		case 3:
			$categ = new Categoria ($db,$_SESSION,$_POST,$_POST['idT'], $opcion);
		...
		case 4:
        	...
        	if( isset($_FILES) && isset($_FILES['image']['name'])){
                ...
                $imagen   = new Imagen ($db,$_SESSION,$_FILES,Comunes::SAVE);
				$idImagen = $imagen->obtenIdImagen();
				$produ = new Producto ($db,$_SESSION,$_POST,$idImagen,$opcion);
				$array = array('exito' => $produ->obtenExito(),'msg' => $produ->obtenMensaje(), 'url' => $url);
			}
			break;
		case 6:
			$testi = new Testimonial($db, $_SESSION, $_POST, $_POST['idT'], $opcion);
			$array = array('exito' => $testi->obtenExito(),'msg' => $testi->obtenMensaje(), 'url' => $url);
			break;
		case 7:
			$pregun = new Preguntas($db, $_SESSION, $_POST, $_POST['idT'], $opcion);
			$array = array('exito' => $pregun->obtenExito(),'msg' => $pregun->obtenMensaje(), 'url' => $url);
			break;

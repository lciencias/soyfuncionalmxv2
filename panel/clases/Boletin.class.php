...
class Boletin extends Comunes{
	...
	function __construct($db,$session,$data,$idImagen,$opc){
		...
		switch($this->opc){
            ...
            case Comunes::MOSTRAR:
				...
				break;
            case Comunes::WEB:
				$this->listarBoletinWebArray();
				break;
            }
	}
	
	private function listarBoletinWebArray(){
		$this->registros = array();
		try{
			$sql = "SELECT a.id,a.email,DATE_FORMAT(a.fecha,'%d-%m-%Y') as fecha,a.status
	...
	private function listar(){
		...
		try{
			$sql = "SELECT a.id,a.email,DATE_FORMAT(a.fecha,'%d-%m-%Y') as fecha,a.status
					FROM ".$this->tabla." as a 
					WHERE a.status = ".Comunes::SAVE." ORDER BY a.fecha ASC;";
			$res = $this->db->sql_query ($sql);			
			if ($this->db->sql_numrows ($res) > 0){
				$this->total = $this->db->sql_numrows ($res);
				while($row = $this->db->sql_fetchass($res)){
		...
		try{
			...
			if(count($this->data) > 0){			
				...
				$ins = "INSERT INTO ".$this->tabla."(email,fecha,status)
						VALUES ('".$this->data['email']."','".$fecha."','".Comunes::LISTAR."');";				
	...
	private function editar(){
		...
		try{
			if($id > 0){
				$this->exito = 1;
				$sql = "SELECT a.id,a.email,DATE_FORMAT(a.fecha,'%d-%m-%Y %H:%i') as fecha,a.status
						FROM ".$this->tabla." as a  
	...
	private function actualizar(){
		$fecha = date("Y-m-d H:i:s");
		try{
			$this->mensaje = "Los datos del boletin no se almacenaron correctamente";
			if(count($this->data) > 0){
				foreach($this->data as $key => $value){
					$this->data[$key] = $this->eliminaCaracteresInvalidos($value);
				}
				$ins = "UPDATE ".$this->tabla." set ";
				$ins .= "email  = '".$this->data['email']."',
						 fecha  = '".$fecha."',
	...
	private function breadcrumb(){
		$this->bread = '<ol class="breadcrumb">
			...
			<li class="active">Bolet&iacute;n</li>
		</ol>';
	}
	...
	private function tabla(){
		...
		if(count($this->registros) > 0){
			...
			foreach($this->registros as $reg){
				...
				<td class="tdCenter">';
					$this->buffer .= '<a href="#" id="e-'.$reg['id'].'-8" class="eliminar">
